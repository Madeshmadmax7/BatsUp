...
export default function TeamManagement({ 
    ...
    useEffect(() => {
        ...
        loadPlayers();
    }, [teams, onError, onPlayersChange]);

    const deleteTeam = async (teamId) => {
        if (!selectedTournament?.id) return;
        onError("");
        try {
            const res = await fetch(`${API_BASE}/api/tournaments/${selectedTournament.id}/remove-team/${teamId}`, { 
                method: "DELETE" 
            });
    ...
    return (
        <>
            <Section 
                title={`Registered Teams${selectedTournament ? ` for "${selectedTournament.tournamentName || selectedTournament.title || selectedTournament.name}"` : ""}`} 

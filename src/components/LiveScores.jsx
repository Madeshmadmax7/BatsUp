...
function ScorecardPopup({ open, onClose, team }) {
    ...
    return (
        ...
        >
            <div
                className="bg-[#181b1f] text-white p-6 rounded-xl max-w-lg w-full"
...
export default function LiveScores() {
    ...
    const currentTeam = selectedMatch && teamIds.includes(String(selectedMatch.teamOneId))
        ? teamsMap[selectedMatch.teamOneId]
        : null;
    const opponentTeam = selectedMatch && teamIds.includes(String(selectedMatch.teamTwoId))
        ? teamsMap[selectedMatch.teamTwoId]
        : null;

    return (
        <div className="bg-black min-h-screen p-8">
            <h1 className="text-3xl font-bold text-white mb-8 text-center">Live Scores</h1>
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-10">
                    {matches.map((match) => (
                        ...
                        >
                            <h2 className="text-lg text-yellow-400 font-bold mb-2">{match.name || `Match #${match.id}`}</h2>
                            <div className="flex items-center justify-between">
                                <div className="text-white font-semibold">{match.teamOneName}</div>
                                <div className="text-gray-400">vs</div>
                                <div className="text-white font-semibold">{match.teamTwoName}</div>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                {selectedMatch ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-white">
                        <section className="bg-[#181b1f] rounded p-6 shadow">
                            ...
                            <div className="font-bold text-yellow-400">
                                Total Runs: {currentTeam?.totalRuns} &nbsp;|&nbsp; Total Wickets: {currentTeam?.totalWickets}
                            </div>
                        </section>
                        ...
                        {opponentTeam && (
                            <section className="bg-[#181b1f] rounded p-6 shadow">
                                ...
                                <div className="font-bold text-yellow-400">Total Wickets: {opponentTeam?.totalWickets}</div>
                            </section>
                        )}
                    </div>
                ) : (

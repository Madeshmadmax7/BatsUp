...
const CricketCards = () => {
    ...
    return (
        <div className="min-h-screen bg-black px-4 sm:px-10 py-10 flex justify-center items-start font-sans">
            <div className="w-full max-w-[1400px] flex flex-col lg:flex-row gap-8 lg:gap-10">

                <div
                    className="w-full lg:w-[600px] h-[450px] rounded-3xl overflow-hidden relative p-6"
                    style={{
                ...
                >
                    ...
                    <div className="relative z-10 flex flex-col justify-center gap-5 h-full w-full max-w-[270px] sm:max-w-full">
                        {newsItems.length === 0 && (
                        ...
                        {newsItems.map((item, idx) => (
                            <div
                                key={idx}
                                className="bg-white/10 backdrop-blur-md border border-white/10 rounded-xl px-4 py-3 flex justify-between items-start hover:bg-white/20 transition cursor-pointer"
                            >
                                <div>
                        ...
                        <div className="mt-6">
                            ...
                            <div className="flex gap-6 overflow-x-auto hide-scrollbar pb-4">
                                {Object.values(teamScores).map((team) => (
                                    <div
                                        key={team.teamName}
                                        className="w-[320px] min-w-[320px] h-[200px] bg-white/10 backdrop-blur-md border border-white/10 rounded-xl p-6 flex flex-col justify-between"
                                    >
                                        <div>
                                            ...
                                            <p className="text-white/80 text-sm mb-1">
                                                Runs: <span className="font-semibold">{team.totalRuns}</span> | Wickets:{" "}
                                                <span className="font-semibold">{team.totalWickets}</span>
                                            </p>
                                        ...
                                        <Link
                                            to={`/scores/${encodeURIComponent(team.teamName)}`}
                                            className="self-end mt-3 bg-yellow-300 text-black px-2 py-0.5 rounded text-[10px] font-semibold hover:bg-yellow-400 transition"
                                        >
                                            SCORE
                                        </Link>
                                    </div>
